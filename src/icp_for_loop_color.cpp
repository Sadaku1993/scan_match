...
typedef pcl::PointXYZRGBNormal PIN;
...
{
	ros::init(argc, argv, "ICP"); //define node name.
	ros::NodeHandle n;
	ros::Rate r(100);
	...
	icp.setMaxCorrespondenceDistance (2.0);//0.3// 対応距離の最大値
	// gicp.setMaxCorrespondenceDistance (10.0);//0.3// 対応距離の最大値
	icp.setMaximumIterations (100);        //20 // ICPの最大繰り返し回数
    ...
    while(ros::ok()){
		...
		if(is_new){
			...
			*cloud_tmp = transformer(*cloud_src,S);
			...
			Eigen::Matrix4f F = icp.getFinalTransformation();
