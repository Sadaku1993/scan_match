...
typedef vector<pcl::PointCloud<pcl::PointXYZINormal> > CloudINs;
...
void removeZeroPoints(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud_in, pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud_out)
{
	size_t size = cloud_in->points.size();
	for(size_t i=0; i<size; ++i){
		if(cloud_in->points[i].x == 0 && cloud_in->points[i].y == 0 && cloud_in->points[i].z == 0) continue;
		pcl::PointXYZINormal tmp;
		tmp = cloud_in->points[i];
...
void merge(const Nodes &nodes, const CloudINs &clouds){
...
int main (int argc, char** argv)
{
	...
	while(fscanf(fp,"%s",s) != EOF){
		...
		if (strcmp(s, "VERTEX_SE3:QUAT") == 0){
			...
			removeZeroPoints(cloud, cloud_out);

			addCloud( shift(node, *cloud_out) );
