...
float offset_dyaw = -0.000185;//tsukuba_all
// float offset_dyaw = 0.00130;//e_top
float pitchrate = 0.0;
float rollrate = 0.0;
float amu_roll = 0.0;
float amu_pitch = 0.0;
...
int main (int argc, char** argv)
{
	...
	/* ros::Subscriber sub = n.subscribe("/cluster/human/removed",1,callback);//人を除いた点群 */
	ros::Subscriber sub = n.subscribe("/cluster/human/removed/normal",1,callback);//人を除いた点群
	// ros::Subscriber sub = n.subscribe("/sq_lidar/points/lcl",1,callback);//人を除いた点群
	// ros::Subscriber sub = n.subscribe("/sq_lidar/points/tf",1,callback);//人を除いた点群
	/* ros::Subscriber sub = n.subscribe("/cloud/lcl/normal",1,callback);//人を除いた点群 */
	...
	// ros::Subscriber amu_sub = n.subscribe("/mavros/imu/data_raw", 1, amu_callback);
	ros::Subscriber amu_sub = n.subscribe("/imu/data", 1, amu_callback);
	// ros::Subscriber amu_sub = n.subscribe("/imu_navio/strapdown", 1, amu_callback);
	// ros::Subscriber odm_sub = n.subscribe("/t_frog/odom", 1, odom_callback);
