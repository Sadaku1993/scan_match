...
//		if(!cloud_src->points.empty()) *cloud_tgt = *cloud_tmp;
	...
	while(ros::ok()){
		...
		//if(is_new){
			...
			if(is_new && seg_img_flag){
				...
				double roll_diff = amu_roll - tgt_roll;
				double pitch_diff = amu_pitch - tgt_pitch;
				Matrix4f R = calcMat(roll_diff, pitch_diff, 0.0);
				*cloud_tmp = transformer(*cloud_src, R);
				toROSMsg(*cloud_tgt, edge.cloud_tgt);
//				toROSMsg(*cloud_src, edge.cloud_src);
				toROSMsg(*cloud_tmp, edge.cloud_src);
				tgt_roll = amu_roll;
				tgt_pitch = amu_pitch;
