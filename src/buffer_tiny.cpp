...
void odom_callback(nav_msgs::Odometry::ConstPtr msg){
	...
	// float last_yaw = tiny_yaw;
	// tiny_yaw = M_PI * msg->twist.twist.angular.z / 180.0;
	float tiny_yaw = msg->twist.twist.angular.z;
	// tiny_yawrate = M_PI * msg->twist.twist.angular.z / 180.0;
	tiny_yawrate = M_PI * tiny_yaw / 180.0;
	// tiny_yawrate = tiny_yaw - last_yaw;
	// cout << "tiny_yawrate = " << tiny_yawrate << endl;
	cout << "tiny_yaw = " << tiny_yaw << endl;
	// while(tiny_yawrate >  M_PI){ tiny_yawrate -= 2.0 * M_PI; }
	// while(tiny_yawrate < -M_PI){ tiny_yawrate += 2.0 * M_PI; }
	// odom_v = msg->twist.twist.linear.x;
...
//	ros::Rate r(100*hosei);
	...
	//		oS(1,3) = odom_v * dt;        
			...
			aS = calcMat(0.0, 0.0, tiny_yawrate*dt); 
			tm.reset();
