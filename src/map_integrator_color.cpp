...
typedef pcl::PointCloud<pcl::PointXYZRGBNormal> CloudIN;
typedef pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr CloudINPtr;
...
CloudIN shift(Eigen::Matrix4f m, CloudIN cloud){
	...
	for(size_t i=0;i<cloud.points.size();i++){
		...
		pnt.normal_y = n(1);
		pnt.normal_z = n(2);
		pnt.r = cloud.points[i].r;
		pnt.g = cloud.points[i].g;
		pnt.b = cloud.points[i].b;
